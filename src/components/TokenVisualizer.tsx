...
export const TokenVisualizer: React.FC<TokenVisualizerProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-emerald-500/40 rounded-xl p-4 sm:p-6 shadow-xl shadow-emerald-950/20 space-y-6">
      {/* Title & Badge */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Trích xuất từ trường: <code className="text-cyan-300 font-mono">{tokenInfo.extractedFromKey}</code> ({tokenInfo.token.length} ký tự)
            </p>
          </div>
        </div>
      ...
      <div>
        ...
        <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 font-mono text-xs text-emerald-400 break-all leading-relaxed max-h-24 overflow-y-auto select-all">
          {tokenInfo.token}
        </div>
      </div>
        ...
        <div className="bg-slate-950/80 rounded-xl border border-slate-800 p-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {tokenInfo.decodedJwt.expiresAtFormatted && (
              <span className={`text-xs flex items-center gap-1 font-mono ${tokenInfo.decodedJwt.isExpired ? 'text-rose-400' : 'text-emerald-400'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="p-3 rounded-lg bg-slate-900 border border-slate-800">
              <span className="text-[10px] font-mono uppercase px-1.5 py-0.5 rounded bg-rose-500/10 text-rose-300 border border-rose-500/20 font-bold">
                1. Header (Thuật toán & Loại)
              </span>
              <pre className="text-xs font-mono text-rose-300 mt-2 whitespace-pre overflow-x-auto">
            ...
            {/* Payload Claims */}
            <div className="p-3 rounded-lg bg-slate-900 border border-slate-800">
              <span className="text-[10px] font-mono uppercase px-1.5 py-0.5 rounded bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 font-bold">
                2. Payload (Claims: Quyền & Thông tin)
              </span>
              <pre className="text-xs font-mono text-cyan-300 mt-2 whitespace-pre overflow-x-auto">
          ...
          <p className="text-[11px] text-slate-400 leading-relaxed">
            💡 <strong>Cơ chế JWT:</strong> Phần 3 là Chữ ký số (Signature). Máy chủ sử dụng khóa bí mật để ký. Vi điều khiển hoặc backend không cần truy vấn Database mỗi lần, chỉ cần kiểm tra chữ ký và hạn <code className="text-cyan-300 font-mono">exp</code> là biết request có hợp lệ hay không.
          </p>
        </div>
      )}

      {/* Educational: How IoT Devices Use This Token */}
      <div className="bg-slate-950/90 rounded-xl border border-cyan-900/40 p-4 sm:p-5 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">
              Trong kiến trúc IoT, sau khi đăng nhập thành công, thiết bị <strong>KHÔNG</strong> gửi lại tài khoản/mật khẩu trong mỗi chu kỳ gửi cảm biến. Thay vào đó, thiết bị đính kèm Token này vào Header <code className="text-cyan-300 font-mono font-semibold">Authorization: Bearer &lt;token&gt;</code>.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
          <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 text-xs">
            <div className="font-semibold text-slate-200 flex items-center gap-1.5 mb-1 text-cyan-300">
              <span>1. Lưu vào RAM / RTC</span>
            </div>
            <p className="text-slate-400 text-[11px]">
              ESP32 gán biến chuỗi <code className="text-slate-300">String authToken = "..."</code> hoặc lưu RTC Memory nếu dùng Deep Sleep.
            </p>
          </div>
          ...
          <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 text-xs">
            ...
            <p className="text-slate-400 text-[11px]">
              Trong lệnh HTTP: <code className="text-slate-300">http.addHeader("Authorization", "Bearer " + authToken);</code>
            </p>
          </div>
          ...
          <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 text-xs">
            ...
            <p className="text-slate-400 text-[11px]">
              Nếu Server trả về <code className="text-rose-400 font-semibold">401 Unauthorized</code>, thiết bị tự động gọi lại hàm Login để lấy Token mới.
            </p>
          </div>
        </div>

        {/* Copyable Authorization Header Box */}
        <div className="p-3 bg-slate-900 rounded-lg border border-slate-800 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 overflow-x-auto text-xs font-mono">
            <span className="text-slate-400">Header cần thêm:</span>
          ...
          >
            {copiedAuthHeader ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
      ...
      <div className="p-4 sm:p-5 bg-slate-950/60 rounded-xl border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-slate-300 block mb-1">
              Nhiệt độ đo được (°C): <strong className="text-amber-300">{telemetryTemp}°C</strong>
            </label>
            <input
              type="range"
              min={10}
          ...
          <div>
            <label className="text-xs text-slate-300 block mb-1">
              Độ ẩm đo được (%): <strong className="text-cyan-300">{telemetryHum}%</strong>
            </label>
            <input
              type="range"
              min={20}
