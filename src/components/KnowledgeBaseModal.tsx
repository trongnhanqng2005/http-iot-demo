...
export const KnowledgeBaseModal: React.FC<KnowledgeBaseModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden transition-colors duration-200">
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/90">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-cyan-100 dark:bg-cyan-500/10 border border-cyan-200 dark:border-cyan-500/30 flex items-center justify-center text-cyan-700 dark:text-cyan-400">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          {/* Navigation Sidebar */}
          <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/60 p-3 space-y-1 overflow-y-auto shrink-0">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-lg text-xs font-medium transition flex items-center justify-between gap-2 ${
                    isActive
                      ? 'bg-cyan-100 dark:bg-cyan-500/15 border border-cyan-300 dark:border-cyan-500/30 text-cyan-900 dark:text-cyan-300 font-semibold'
                ...
                >
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-900 text-slate-700 dark:text-slate-400 border border-slate-300 dark:border-slate-800 shrink-0">
                    {topic.badge}
                  </span>
                </button>
              );
          ...
          {/* Content Area */}
          <div className="flex-1 p-5 sm:p-6 overflow-y-auto space-y-4 bg-white dark:bg-slate-900">
            {selectedTopicId === 'status-codes' ? (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-800 dark:text-slate-300 font-semibold">{st.meaning}</span>
                        </div>
                      </div>
                      <p className="text-slate-600 dark:text-slate-400 pl-1 border-l-2 border-slate-300 dark:border-slate-700 text-[11px] leading-relaxed">
                        <strong className="text-cyan-700 dark:text-cyan-300">Xử lý trong IoT:</strong> {st.iotContext}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold px-2 py-0.5 rounded bg-cyan-100 dark:bg-cyan-500/20 text-cyan-800 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-500/30">
                      {activeTopic.badge}
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{activeTopic.summary}</p>
                </div>

                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 leading-relaxed max-w-none">
                  <div
                    dangerouslySetInnerHTML={{
                      __html: activeTopic.content
                        .replace(/### (.*)/g, '<h5 class="text-sm font-bold text-cyan-700 dark:text-cyan-300 mt-4 mb-2">$1</h5>')
                        .replace(/## (.*)/g, '<h4 class="text-base font-bold text-slate-900 dark:text-slate-100 mt-4 mb-2">$1</h4>')
                        .replace(/\*\*(.*?)\*\*/g, '<strong class="text-slate-900 dark:text-slate-100 font-semibold">$1</strong>')
                        .replace(/`([^`]+)`/g, '<code class="text-amber-700 dark:text-amber-300 font-mono bg-slate-200 dark:bg-slate-900 px-1 py-0.5 rounded">$1</code>')
                        .replace(/\| (.*) \|/g, (match) => {
        ...
        {/* Footer */}
        <div className="p-3 sm:p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
