...
export const RequestResponseInspector: React.FC<RequestResponseInspectorProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm dark:shadow-lg dark:shadow-black/20 flex flex-col transition-colors duration-200">
        {/* Header bar */}
        <div className="p-3.5 sm:p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 flex flex-wrap items-center justify-between gap-2">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded font-medium transition ${
                requestTab === 'formatted'
                  ? 'bg-white dark:bg-cyan-950/80 text-cyan-800 dark:text-cyan-300 border border-slate-200 dark:border-cyan-800/60 shadow-xs'
        ...
        <div className="p-4 flex-1 flex flex-col">
          {request ? (
            <div className="space-y-3 flex-1 flex flex-col">
              ...
              <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-2 text-xs font-mono">
                <div className="flex items-center gap-2 overflow-x-auto">
                  <span className="px-2 py-0.5 rounded bg-cyan-100 dark:bg-cyan-500/20 text-cyan-800 dark:text-cyan-300 font-bold border border-cyan-200 dark:border-cyan-500/30">
                    {request.method}
                  </span>
                  <span className="text-slate-700 dark:text-slate-300 truncate">{request.url}</span>
                </div>
                <button
              ...
              {requestTab === 'formatted' && (
                <div className="flex-1 flex flex-col">
                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-1.5">
                    <span className="font-semibold text-slate-700 dark:text-slate-300">Nội dung JSON Body:</span>
                    <div className="flex items-center gap-2">
                      <button
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 ml-2"
                      >
              ...
              {requestTab === 'raw' && (
                <div className="flex-1 flex flex-col">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2">
                    💡 Chuẩn HTTP 1.1 yêu cầu phân cách mỗi dòng header bằng ký tự CRLF (<code className="text-amber-600 dark:text-amber-300">\r\n</code>), và 2 lần CRLF trước khi bắt đầu Body.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-slate-400 dark:text-slate-500 border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
              ...
              <p className="text-xs font-medium text-slate-600 dark:text-slate-400">Chưa có HTTP Request nào được gửi.</p>
        ...
        <div className="p-3.5 sm:p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800 text-xs">
            <button
              ...
              className={`px-2.5 py-1 rounded font-medium transition ${
                responseTab === 'body'
                  ? 'bg-white dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300 border border-slate-200 dark:border-emerald-800/60 shadow-xs'
                  : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
              }`}
            >
              Response Body
        ...
        <div className="p-4 flex-1 flex flex-col">
          {isLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">Đang chờ máy chủ phản hồi...</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-500 mt-1">Đang xử lý bắt tay và chứng thực</p>
            </div>
          ) : response ? (
            <div className="space-y-3 flex-1 flex flex-col">
              {/* Status Code Banner */}
              <div
                className={`p-3 rounded-lg border flex flex-col gap-2 ${
                  response.status >= 200 && response.status < 300
                    ? 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-500/40 text-emerald-900 dark:text-emerald-300'
                    : response.status >= 400 && response.status < 500
                    ? 'bg-amber-50 dark:bg-amber-950/40 border-amber-200 dark:border-amber-500/40 text-amber-900 dark:text-amber-300'
                    : 'bg-rose-50 dark:bg-rose-950/40 border-rose-200 dark:border-rose-500/40 text-rose-900 dark:text-rose-300'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold px-2 py-0.5 rounded bg-black/10 dark:bg-black/30">
                      {response.statusText || (response.isError ? 'Connection Error' : 'Unknown')}
                    </span>
                  </div>
                  <span className="text-xs font-mono text-slate-600 dark:text-slate-300">{response.durationMs} ms</span>
                </div>

                {/* Educational status explanation */}
                {statusExplanation ? (
                  <div className="text-xs text-slate-700 dark:text-slate-300 border-t border-slate-200 dark:border-slate-700/50 pt-2 space-y-1">
                    <p>
                  ...
                  <div className="text-xs text-rose-800 dark:text-rose-300 border-t border-rose-200 dark:border-rose-800/40 pt-2 space-y-1">
                    ...
                    {response.errorType === 'CORS_ERROR' && (
                      <div className="text-[11px] text-amber-800 dark:text-amber-300 bg-amber-100/70 dark:bg-amber-950/40 p-2 rounded border border-amber-300 dark:border-amber-800/50 mt-1">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-slate-400 dark:text-slate-500 border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
              ...
              <p className="text-xs font-medium text-slate-600 dark:text-slate-400">Chưa có phản hồi từ máy chủ.</p>
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">
